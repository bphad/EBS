...
public class Account {
	...
	public void addEmployee(Employee e) {
		...
		jdbc.update(
				"insert into employee(emp_name,emp_email,emp_address,emp_joindate,emp_leavedate,emp_salary,emp_job_title,username)"
						+ "values (:emp_name,:emp_email,:emp_address,:emp_joindate,:emp_leavedate,:emp_salary,:emp_job_title,:username)",
				map);
	...
	public void addProjet(Project p) {
		...
		jdbc.update(
				"insert into project(proj_name,proj_start_date,proj_estimated_end_date,proj_end_date,proj_client_name,mid,budget,status)"
						+ "values (:proj_name,:proj_start_date,:proj_estimated_end_date,:proj_end_date,:proj_client_name,:manager_id,:budget,:status)",
				map);
	...
	public void unassignEmployeeFromProject(String eid, String pid) {
		// Delete eid, pid from employee_has_project table
		MapSqlParameterSource map = new MapSqlParameterSource();
		map.addValue("eid", eid);
		map.addValue("pid", pid);
		
		jdbc.update(" delete from employee_has_project where emp_id=:eid and proj_id=:pid ", map);
	}
	...
	public void incrementSalary(String eid, String sal) {
		...
		float s1 = s / 100; // 0.02
		map.addValue("eid", eid);
		map.addValue("salary", s1);
		jdbc.update("update employee set emp_salary= ((emp_salary * :salary)/100) + emp_salary  where emp_id=:eid",
				map);
	 ...
	 * jdbc.query("select * from product", new RowMapper<Product>(){
	 * 
	 * @Override public Product mapRow(ResultSet rs, int rowNum) throws SQLException
	 * { Product p =new Product(); p.setId(rs.getInt("id"));
	 * p.setName(rs.getString("name")); p.setPrice(rs.getString("price"));
	 * p.setDesc(rs.getString("description")); return p; }
	 * 
	 * });
	 * 
	 * }
	 * 
	 * 
	 * @SuppressWarnings("unchecked") public List<Product> getAllProducts(){ return
	 * sessionFactory.getCurrentSession().createQuery("from Product").list(); }
	 * 
	 * public Product getSingleProduct(int id){ MapSqlParameterSource map = new
	 * MapSqlParameterSource(); map.addValue("id", id); return
	 * jdbc.queryForObject("select * from product where id=:id", map ,new
	 * RowMapper<Product>(){
	 * 
	 * @Override public Product mapRow(ResultSet rs, int rowNum) throws SQLException
	 * { Product p =new Product(); p.setId(rs.getInt("id"));
	 ...
	 * }); }
	 * 
	 * public void addEmployee(Student s){ //BeanPropertySqlParameterSource param =
	 * new BeanPropertySqlParameterSource(v); MapSqlParameterSource map = new
	 * MapSqlParameterSource(); map.addValue("name", s.getName());
	 * map.addValue("contact", s.getContact()); map.addValue("email", s.getEmail());
	 * map.addValue("password", s.getPassword()); map.addValue("p1",
	 * passenc.encode(s.getPassword())); map.addValue("authority", "student");
	 * map.addValue("enabled", "true");
	 * 
	 * jdbc.update("insert into employee(name,contact,email,password) " +
	 * "values (:name,:contact,:email,:password)", map); jdbc.
	 * update("insert into users(username,password,enabled) values (:email,:p1,:enabled)"
	 * , map); jdbc.
	 * update("insert into authorities(username,authority) values (:email,:authority)"
	 * , map);
	 * 
	 * }
	 * 
	 ...
	 * MapSqlParameterSource map = new MapSqlParameterSource();
	 * map.addValue("email", currentUsername); return
	 * jdbc.queryForObject("select * from employee where email=:email", map ,new
	 * RowMapper<Student>(){
	 * 
	 * @Override public Student mapRow(ResultSet rs, int rowNum) throws SQLException
	 * { Student s =new Student(); s.setId(rs.getInt("id"));
	 * s.setName(rs.getString("name")); s.setContact(rs.getString("contact"));
	 * s.setEmail(currentUsername); return s; }
	 * 
	 * });
	 * 
	 * 
