				...
				<div class="row">
					<div class="col-lg-12 ">
						...
						<ul class="nav nav-tabs">
							<li class="active"><a href="#record" data-toggle="tab">Current Salary</a></li>
							<li class=""><a href="#increment" data-toggle="tab">Increments</a></li>
							<li class=""><a href="#estimate" data-toggle="tab">Project Estimates</a></li>
						</ul>
						<div class="tab-content">
								...
								<p>
									<c:forEach var="e" items="${emp_list }">
										<font size="4" color="grey">
											<c:out value="${e.getName() }"></c:out></font><BR>
											Salary: Rs.<c:out value="${e.getSalary() }"/><BR>
											Job Title:<c:out value="${e.getJob_title()}"/><BR>
										<a class="btn btn-warning" href="${pageContext.request.contextPath }/showIncrementReport?eid=<c:out value="${e.getId() }"></c:out>">Show Increment Report</a><hr>
									</c:forEach>
								</p>
							</div>
							<div class="tab-pane fade" id="increment">
								...
								<form method="post" action="${pageContext.request.contextPath  }/IncrementSalary">
									<p>
										Select Employee: <select name="eid" class="form-control">
											<c:forEach var="e" items="${emp_list }">
												<option value='<c:out value="${e.getId() }"/>'><c:out value="${e.getName() }" /> --
													<c:out value="${e.getJob_title() }" /></option>
											</c:forEach>
										</select>
										Select Increment: <select name="increment_sal" class="form-control">
							...
							<div class="tab-pane fade" id="estimate">
								...
								<p>
									give option for batch increments to entire project team
