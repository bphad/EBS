					...
					<div class="col-lg-12 ">
						...
						<c:forEach var="e" items="${list_search }">
							<h4>
								...
								<c:if test="${e.getLeave_date() != null }">**past employee</c:if>
							</h4>
								...
								Salary: <c:out value="${e.getSalary() }" /><BR>
								Job Title: <c:out value="${e.getJob_title() }" />
						</c:forEach>
						<c:forEach var="p" items="${list_searchp}">
							<h4>
								Project Id : <c:out value="${p.getId()}"/><br>
								Project Name : <c:out value="${p.getName()}"/><br>
								Project Client : <c:out value="${p.getClient_name()}"/><br>
							</h4>
							<a class="btn btn-warning" href="${pageContext.request.contextPath }/show-emp-on-proj?pid=<c:out value="${p.getId()}"></c:out>">Show Employee</a><hr>
						</c:forEach>
					</div>
				</div>
			</div>
